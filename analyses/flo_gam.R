...
catch_data = get_ecomon_catch_data("limfer_100m3", "../auxdata/catch-data/ecomon_flounder.csv")
...
ggplot(catch_data)+geom_point(aes(x=sfc_temp,y=catch))+
  labs(x="Temperature (˚C)", y="Catch (fish/100m^3)", title = "Flounder - Catch vs Sea Surface Temperature")
ggplot(catch_data)+geom_point(aes(x=sfc_salt,y=catch))+
  labs(x="Salinity (ppm)", y="Catch (fish/100m^3)", title = "Flounder - Catch vs Sea Surface Salinity")
ggplot(catch_data)+geom_point(aes(x=depth,y=catch))+
  labs(x="Temperature (˚C)", y="Catch (fish/100m^3)", title = "Flounder - Catch vs Bathymetric Depth")
ggplot(catch_data)+geom_bar(aes(x=btm_sub, weight=catch))+
  labs(x="Substrate Type", y="Catch (fish/100m^3)", title = "Flounder - Catch vs Substrate")

# Plot points in space to visualize geographic distribution
ggplot(catch_data,aes(x=easting,y=northing, weight=catch))+geom_bin2d(bins=100)+ggtitle("Flounder Catch")
...
pred_data = read.csv("../auxdata/environmental_data_198605.csv")
...
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Flounder -  - THS")

# -------------- Make new predictions on THS model -------------- #

predicted = predict_data(gam_th, pred_data)
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Flounder -  - TH")
