...
catch_data = get_ecomon_catch_data("gadmor_100m3", "../auxdata/catch-data/ecomon_cod.csv")
...
ggplot(catch_data,aes(x=easting,y=northing, weight=catch))+geom_bin2d(bins=100)+ggtitle("Atlantic Cod Catch")
...
aic_scores = AIC(gam_th, gam_ths, gam_thf, gam_lalo_th, gam_lalo_ths)
...
temp_plot+ylim(-10, 10) + xlim(0, 25) + labs(x="Surface Temperature (˚C)", title="Temperature Spline") 
...
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Atlantic Cod - THS")

# -------------- Make new predictions on THS model -------------- #

predicted = predict_data(gam_th, pred_data)
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Atlantic Cod - TH")
