...
catch_data = get_ecomon_catch_data("scosco_100m3", "../auxdata/catch-data/ecomon_mackerel.csv")
...
pred_data = read.csv("../auxdata/environmental_data_198607.csv")
...
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Atlantic Mackerel - THS")

# -------------- Make new predictions on THS model -------------- #

predicted = predict_data(gam_th, pred_data)
ggplot(predicted)+geom_point(aes(x=easting, y=northing, colour=log(floored)), size=0.001)+
  scale_colour_gradient2(low="blue", high="red", mid="white")+ggtitle("Atlantic Mackerel - TH")
