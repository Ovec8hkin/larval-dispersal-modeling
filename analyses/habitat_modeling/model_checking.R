...
model_ts = catch~s(sfc_temp)+s(sfc_salt)
model_ths = catch~s(depth)+s(sfc_temp)+s(sfc_salt)
model_thf = catch~s(depth)+s(sfc_temp)+btm_sub

models = list(model_t, model_h, model_th, model_ts, model_ths, model_thf)
dataframes = c()
for(f in fish_datasets){
  aic_scores = c()
  df = data.frame(row.names=c("temp", "depth", "temp-depth", "temp-salt", "temp-depth-salt", "temp-depth-floor"))
  for(m in models){
